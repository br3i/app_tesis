...
def show_df_documents():
    ...
    placeholder_info.info(":material/info: Por favor, edita los documentos antes de guardar los cambios.")
...
#!!!!!!!!!!!!!!!!!!!!!COLOCAR ATRIBUTOS FALTANTES EN TODAS LAS COLUMNAS!!!!!!!!!!!!!!!!!!
        ...
        with st.spinner("Cargando archivos..."):
            edited_df = st.data_editor( 
                ...
                column_config={
                    "name": st.column_config.TextColumn("Nombre del Documento", required=True, width="medium", max_chars=100),
                    "collection_name": st.column_config.SelectboxColumn(label="Colección",options=collections_names,width="small",help="Nombre de la colección a la que pertenece el documento",required=True,pinned=False, 
                    ),
                    "created_at": st.column_config.DatetimeColumn(label="Fecha de creación", width=None, help=None, required=True, min_value=None, max_value=None, step=None, timezone=None),
                    "location": st.column_config.TextColumn("Ubicación del Archivo", width=None, disabled=True, required=True, max_chars=100),
                    "view": st.column_config.LinkColumn("Ver", width="small", display_text="💻", required=True, disabled=True, max_chars=100),
                },
                hide_index=True,
        ...
        for index, row in edited_df.iterrows():
            original_row = documents_df.iloc[index]  # Filas originales

            # Compara cada columna para ver si hubo algún cambio
            if (row["name"] != original_row["name"] or
                row["collection_name"] != original_row["collection_name"] or
                row["created_at"] != original_row["created_at"]):
                ...
                if rows_to_update:
                    with st.spinner("Guardando cambios..."):     
                        for row in rows_to_update:
                            document_id = documents[row["index"]]['id']  # Supone que el `id` está disponible
                            data = {
                                "name": row["name"],
                                "collection_name": row["collection_name"],
                                "created_at": row["created_at"],
                            }
                            response = requests.put(f"{BACKEND_URL}/edit_document/{document_id}", data=data)
                            if response.status_code == 200:
                                    placeholder_success.success(f"¡Documento '{row['name']}' actualizado correctamente!")
                            else:
                                placeholder_error.error(f"Error al actualizar el documento '{row['name']}'.")
                    get_documents_from_db.clear()
                    get_collections.clear()
                    documents = get_documents_from_db()
                    collections = get_collections()
                else:
                    placeholder_warning.warning("No se detectaron cambios para actualizar.")
        with cols[1]:

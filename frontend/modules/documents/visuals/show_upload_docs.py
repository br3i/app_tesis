...
MAX_FILES_UPLOAD = st.secrets.get('MAX_FILES_UPLOAD', 'Not found')
...
def show_upload_docs():
    ...
    placeholder_info.info(":material/info: Puedes subir hasta " + str(MAX_FILES_UPLOAD) + " archivos cada vez")
    uploaded_files = st.file_uploader("Elige un archivo PDF", type=['pdf'], accept_multiple_files=True, help="Seleccione los archivos", key=st.session_state.uf_key)
    ...
    if uploaded_files:
        if len(uploaded_files) > MAX_FILES_UPLOAD:
            ...
            with st.spinner("Cargando..."):
                with st.expander("Documentos seleccionados"):
                    for uploaded_file in uploaded_files:
                        with st.container():
                            st.write(f":material/upload_file: :orange[Nombre:] :violet[{uploaded_file.name}]")
                            st.write(f":material/cloud_upload: :orange[Tamaño:] :violet[{uploaded_file.size / 1024:.2f} KB]")
            ...
            if not collections:
                ...
                collection_name = st.text_input("Ingresa un nombre para una nueva colección")
            else:
                ...
                if collection_name:
                    for uploaded_file in uploaded_files:
                        with st.spinner(f"Procesando '{uploaded_file.name}'..."):
                            ...
                            response = requests.post(f"{BACKEND_URL}/document", data=data, files=files)
                        ...
                        try:
                            ...
                            if status == "Successfully Uploaded":
                                ...
                                with total_success_placeholder.container():
                                    st.success(f":material/check: {success_count} archivo(s) subido(s) correctamente.")
                                ...
                                with success_placeholder.popover(":material/check: Detalle:"):
                                    if success_messages:
                                ...
                                with total_error_placeholder.container():
                                    st.error(f":material/gpp_maybe: Se encontraron {error_count} error(es).")
                                error_messages.append(f":material/gpp_maybe: Error al subir :red-background['{uploaded_file.name}]': {message}")
                                with error_placeholder.popover(":material/gpp_maybe: Detalle:"):
                                    if error_messages:
                            ...
                            with total_error_placeholder.container():
                                    st.error(f":material/gpp_maybe: Se encontraron {error_count} error(es).")
                            error_messages.append(f":material/gpp_maybe: Error al procesar la respuesta del servidor para :red-background['{uploaded_file.name}'].")
                            with error_placeholder.popover(":material/gpp_maybe: Detalle:"):
                                if error_messages:
                                    for error_message in error_messages:
                                        st.write(f"- {error_message}")
                        
                    show_toast("El proceso de los archivos terminó", icon=":material/check:")
                    time.sleep(2)

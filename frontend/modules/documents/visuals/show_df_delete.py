...
def show_df_delete():
    ...
    placeholder_info.info(":material/info: Por favor, selecciona los documentos que deseas eliminar.")
    ...
    if documents:
        ...
        for index, row in selected_df.iterrows():
            if row["Eliminar"]:  # Si el checkbox está marcado
                document_id = documents_df.iloc[index]['id']
                    ...
                    with st.spinner("Eliminando documentos..."):     
                        for row in rows_to_delete:
                            ...
                            if response.status_code == 200:
                                placeholder_success.success(f":material/check: ¡Documento '{documents[row['index']]['name']}' eliminado correctamente!")
                            else:
                                st.error(f":material/gpp_maybe: Error al eliminar el documento '{documents[row['index']]['name']}'.")
                    get_documents_from_db.clear()
                    documents = get_documents_from_db()
                    st.rerun()
                else:
                    placeholder_warning.warning("⚠️ No se detectaron documentos seleccionados para eliminar.")
        with cols[1]:
            st.button("Restaurar Valores", on_click=reset_df_dlt)  # Botón para restaurar si se desea
    else:

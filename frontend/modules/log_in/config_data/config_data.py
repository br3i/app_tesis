...
CONFIG_FILE = os.path.abspath(os.path.join(current_dir, '..', '..', '..', 'config', 'config.yaml'))
...
def update_users_in_yaml():
    ...
    if users:
        config = load_config()
        id_mapping = {v.get('id'): k for k, v in config['credentials']['usernames'].items()}
        ...
        for user in users:
            ...
            if user_id in id_mapping:
                ...
                config['credentials']['usernames'][current_username] = {
                    ...
                    'logged_in': config['credentials']['usernames'][current_username].get('logged_in', False),
                    'last_login': config['credentials']['usernames'][current_username].get('last_login', None),
                    'roles': user['roles'],
                ...
                if current_username != user['username']:
                    config['credentials']['usernames'][user['username']] = config['credentials']['usernames'].pop(current_username)
            else:

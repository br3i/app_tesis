...
def validate_register_user(
    ...
    return register_user(
        first_name_new, last_name_new, username_new, email_new, password_new, roles
    )
...
def show_form_register(roles):
    ...
    col1, col2 = frm_register_user.columns(
        [0.5, 0.5], gap="small", vertical_alignment="center"
    )
    with col1:
        first_name_input = st.text_input(
            "Primer nombre",
            max_chars=MAX_FIRSTNAME_LENGTH,
            help="Ingrese su primer nombre",
    ...
    col3, col4 = frm_register_user.columns(
        [0.5, 0.5], gap="small", vertical_alignment="center"
    )

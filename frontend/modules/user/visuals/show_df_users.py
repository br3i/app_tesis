...
def convert_int64_to_int(value):
    if isinstance(value, (np.int64, pd.Timestamp)):
        return int(value)
    return value
...
def show_df_users():
    ...
    placeholder_info.info(":material/info: Si desea editar roles, hagalo primero y actualice la tabla.")
    ...
    if users:
        ...
        with st.expander("Edición de Roles"):
            ...
            if st.button("Actualizar Tabla", type="primary"):
                ...
                placeholder_success.success(f":material/check: Roles del usuario '{selected_user}' actualizados correctamente.")
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "roles": st.column_config.ListColumn(label="Rol",width="small",help="Rol asigando al usuario",),
            },
            hide_index=True,
        ...
        with cols[0]:
            ...
            if st.button("Guardar Cambios", type="primary"):
                ...
                if not differences_roles.empty or not differences.empty:
                    placeholder_info.info(":material/info: Se encontraron diferencias entre los datos editados y los originales. Actualizando...")
                    ...
                    for idx in set(differences_roles.index).union(differences.index):
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                placeholder_success.success(f":material/check: Usuario {response_data['username']} actualizado correctamente.")
                            else:
                                st.error(f":material/gpp_maybe: Error al actualizar el usuario con ID {id_usuario}.")
                        except requests.exceptions.RequestException as e:
                            placeholder_error.error(":material/gpp_maybe: Hubo un error al realizar la solicitud de actualización.")
                    ...
                    show_toast("Todos los datos han sido actualizados", icon=":material/check:")   
                    time.sleep(2)
                    ...
                    placeholder_warning.warning("⚠️ No hay cambios entre los datos editados y los originales.")
        with cols[1]:
            if st.button("Restaurar/Recargar", on_click=reset_df_user):
                ...
                st.session_state['df_u_key'] = str(uuid.uuid4())  # Generar nueva clave para evitar cache
                st.session_state['original_roles_df'] = users_df.copy()  # Guardar el estado original

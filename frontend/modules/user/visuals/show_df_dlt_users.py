...
def show_df_dlt_users():
    ...
    placeholder_error.error(":material/gpp_maybe: Asegurese de seleccionar solo los usuarios que desea eliminar.")
    ...
    if users:
        ...
        with st.spinner("Cargando usuarios..."):
            selected_df = st.data_editor(
                ...
                column_config={
                    ...
                    "username": st.column_config.TextColumn("Usuario", disabled=True, width="small", max_chars=MAX_USERNAME_LENGTH),
                    "email": st.column_config.TextColumn("Email", disabled=True, width="small"),
                    "first_name": st.column_config.TextColumn("Primer Nombre", disabled=True, width="small"),
                    "last_name": st.column_config.TextColumn("Apellido", disabled=True, width="small"),
        ...
        for index, row in selected_df.iterrows():
            if row["Eliminar"]: 
                user_id = users_df.iloc[index]['id']
                ...
                if rows_to_delete:
                    with st.spinner("Eliminando usuarios..."):     
                        for row in rows_to_delete:
                            user_id = row["user_id"]
                            response = requests.delete(f"{BACKEND_URL}/delete_user/{user_id}")
                            if response.status_code == 200:
                                placeholder_success.success(f":material/check: Usuario '{users[row['index']]['username']}' eliminado correctamente!")
                                sync_deleted_users()
                            else:
                                st.error(f":material/gpp_maybe: Error al eliminar el usuario '{users[row['index']]['username']}'.")
                    load_users.clear()
                    users = load_users()
                    st.rerun()
                else:
                    placeholder_warning.warning("⚠️ No se detectaron usuarios seleccionados para eliminar.")
        with cols[1]:

...
def show_create_users():
    ...
    if on == ":orange[Subir archivo CSV]":
        uploaded_files = st.file_uploader("Elige un archivo", type=['csv'], accept_multiple_files=True, help="Seleccione los archivos", key=st.session_state.uu_key)
        if uploaded_files:
    ...
    if on == "Formulario":
        ...
        try:
            ...
            with st.container(border=True):
                selection_roles = st.pills("**Roles**",translated_roles, selection_mode="multi", default=["Usuario"])
            placeholder_info = st.empty()
            selected_roles_in_english = [options_roles[translated_roles.index(role)] for role in selection_roles]
            #st.markdown(f"Your selected options: {selected_roles_in_english}.")

            if selected_roles_in_english:
                print("Tipo de selected_roles_in_english: ", type(selected_roles_in_english))
                username_new, status_register, message = show_form_register(selected_roles_in_english)
                if status_register == "success":
                    placeholder_info.success(f"{message}: {username_new}", icon=":material/check:")
                    load_users.clear()
                ...
                placeholder_info = st.error("Por favor seleccione al menos un Rol",icon=":material/gpp_maybe:")

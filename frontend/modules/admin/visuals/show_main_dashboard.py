...
MAX_NOTIFICATIONS_HEIGHT = st.secrets.get("MAX_NOTIFICATIONS_HEIGHT", "Not found")

def show_main_dashboard():
    st.header('Dashboard de :orange[Administración] :material/admin_panel_settings:', divider="orange")
    ...
    with cols[0]:
        if st.button(":material/account_box: Perfil", key="btn_profile", use_container_width=True):
            st.switch_page("pages/profile.py")
    
    with cols[1]:
        if st.button(":material/group: Usuarios", key="btn_users", use_container_width=True):
            st.switch_page("pages/users.py")

    with cols[2]:
        if st.button(":material/folder: Documentos", key="btn_documents", use_container_width=True):
            st.switch_page("pages/documents.py")
    
    with cols[3]:
        if st.button(":material/bar_chart: Reportes", key="btn_reports", use_container_width=True):
                ...
                with st.container(border=True):
                    with st.spinner("Cargando Notificación"):
                        ...
                        if any(role in user_roles for role in notification['roles']):
                            ...
                            created_at = datetime.strptime(notification['created_at'], "%Y-%m-%dT%H:%M:%S.%f")
                            ...
                            if kind == "urgent":
                                st.markdown(f"_Creada el :red-background[{formatted_datetime}]_")
                                st.error(f"⚠️ **{title}**: {message}")
                            elif kind == "normal":
                                st.markdown(f"_Creada el :green-background[{formatted_datetime}]_")
                                st.success(f"ℹ️ **{title}**: {message}")
                            elif kind == "reminder":
                                st.markdown(f"_Creada el :orange-background[{formatted_datetime}]_")
                                st.warning(f"🔔 **{title}**: {message}")
                            elif kind == "informative":
                                st.markdown(f"_Creada el :blue-background[{formatted_datetime}]_")
                                st.info(f"📢 **{title}**: {message}")
                            elif kind == "alert":
                                st.markdown(f"_Creada el :red-background[{formatted_datetime}]_")
                                st.error(f"🚨 **{title}**: {message}")
                            else:
                                st.markdown(f"_Creada el :gray-background[{formatted_datetime}]_")
    ...
    with col5:
        placeholder_docs_consulted = st.empty()  # Otro espacio vacío para los documentos

...
def save_embeddings(chunk, collection, document_metadata, id_document, db):
    ...
    try:
        ...
        if not embedding:
            raise ValueError("Error al generar el embedding: el resultado está vacío o no es válido.")
        ...
        print(f"Embedding guardado para el fragmento {document_metadata['chunk_index']}.")
        ...
        try:
            ...
            if not document:
                raise ValueError(f"Documento con ID {id_document} no encontrado en la base de datos.")
            ...
            document.embeddings_uuids = func.array_append(document.embeddings_uuids, str(document_metadata['uuid']))
            ...
            print(f"Documento actualizado en embedding_uuid {document_metadata['chunk_index']}.")

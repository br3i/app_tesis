...
class Code(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.now(pytz.timezone(TIME_ZONE)))  # Fecha de creación del código
    ...
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False)  # Relación con el modelo User
    ...
    def is_expired(self):
        ...
        return self.expires_at < datetime.now(pytz.timezone(TIME_ZONE)) if self.expires_at else False
    
    def __init__(self, **kwargs):
        # Asignar automáticamente expires_at si no se proporciona
        if 'expires_at' not in kwargs:
            kwargs['expires_at'] = datetime.now(pytz.timezone(TIME_ZONE)) + timedelta(minutes=15)

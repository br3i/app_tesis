...
class Metric(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)  # Identificador único de la métrica
    ...
    memory_process = Column(Float, nullable=False)  # Uso de memoria después del procesamiento
    memory_final = Column(Float, nullable=False)  # Uso de memoria final
    created_at = Column(DateTime, default=lambda: datetime.now(pytz.timezone(TIME_ZONE)))  # Fecha de creación de la métrica

    #document = relationship("Document", back_populates="metrics")  # Relación con el modelo Document
    associations = relationship("MetricAssociation", back_populates="metric", cascade="all, delete-orphan")

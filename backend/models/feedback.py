...
class Feedback(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)  # Identificador único del feedback
    ...
    full_response = Column(String, nullable=False)  # El texto completo del documento consultado
    sources = Column(JSON, nullable=False)  # Fuentes relacionadas con el feedback
    use_considerations = Column(Boolean, nullable=False) # Consideraciones utilizadas para la respuesta
    n_documents = Column(Integer, nullable=False) # Número de documentos consultados
    word_list = Column(JSON, nullable=False) # Palabras clave utilizadas para la consulta
    ...
    text = Column(String, nullable=True)  # Texto adicional opcional proporcionado por el usuario

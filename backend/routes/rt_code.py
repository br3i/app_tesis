...
def get_code_by_field(field_name: str, value: str, db: Session) -> Code:
    code = db.query(Code).filter(getattr(Code, field_name) == value).first()
    if not code:
        raise HTTPException(status_code=404, detail=f"Código no encontrado por {field_name}: {value}")
    return code
...
def update_code_status_in_db(code: Code, status: CodeStatus, db: Session) -> None:
    ...
    if status == CodeStatus.used:
        code.used_at = datetime.now(tz)  # Si el estado es 'usado', registrar la fecha de uso
    db.commit()
...
@router.post("/create-code", response_model=CodeResponse)
...
async def verify_code(data: VerifyCodeRequest):
    ...
    try:
        ...
        if code_data.status != CodeStatus.active:
        ...
        if code_data.expires_at.tzinfo is None:
            code_data.expires_at = tz.localize(code_data.expires_at)
...
async def update_code_status(code_id: int, request: UpdateCodeStatusRequest):
    ...
    try:
        ...
        return {"success": True, "message": f"Estado del código {code_id} actualizado a {request.status}"}
    finally:
        db.close()  # Asegura que se cierre la sesión
